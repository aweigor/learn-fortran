...
contains
   ...
   subroutine readVector(inputFile,size,fmt,vector)
      character(*), intent(in) :: inputFile, fmt
      ...
      open (file=inputFile, newunit=In)
         read (In, '('//size//fmt//')', end=100) vector(:)
         return
         100 call createVector(inputFile,size,fmt,vector)
      close (In)
      
   end subroutine readVector

   subroutine createVector(inputFile,size,fmt,vector)
      character(*), intent(in)   :: inputFile, fmt
      integer(I_), intent(in)    :: size
      real(R_)                   :: vector(:)
      integer                    :: Out = 1
